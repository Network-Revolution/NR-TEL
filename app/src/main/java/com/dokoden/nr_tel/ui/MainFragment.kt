...
class MainFragment : Fragment() {
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View {
        ...
        val sipStack by viewModels<SipStackViewModel>()
        ...
        val menuMap = mutableMapOf<MenuItem, String>()
...
//        binding.tabLayout.getTabAt(1)?.select()
        ...
        sipStack.registeredAccount.observe(viewLifecycleOwner) { regAcc ->
            binding.navView.menu.forEach {
                if (regAcc.contains(menuMap[it])) {
                    it.setIcon(R.drawable.ic_indicator_circle)
                }
            }
        }
...
//                val menuItem = binding.navView.menu.add(0, acountData.orderNumber, 0, "")
                val menuItem = binding.navView.menu.add(0, acountData.orderNumber, 0, acountData.titleText)
                menuItem
                    .setCheckable(true)
                    .setChecked(acountData.isDefault == 1 || it.size == 1)
                    .setIcon(R.drawable.ic_indicator_cross)
                    .setOnMenuItemClickListener { item ->
                        accountViewModel.updateOutgoing(item.itemId)
                        return@setOnMenuItemClickListener true
                    }
...
//                        switch.text = acountData.titleText
                ...
                menuMap[menuItem] = sipStack.makeAccountUri(acountData)
            }
        }
        ...
        binding.also {
            ...
            it.bottomNavigationView.setOnItemSelectedListener { menuItem ->
                ...
                override fun onTabSelected(tab: TabLayout.Tab) {
                    when (tab.position) {
                        0 -> {
                            ...
                            it.mainFab.also { mainFab ->
                                mainFab.hide()
                                mainFab.setImageResource(R.drawable.ic_call_black_24dp)
                                mainFab.show()
                            }
                        }
                        1 -> {
                            ...
                            it.mainFab.also { mainFab ->
                                mainFab.hide()
                                mainFab.setImageResource(R.drawable.ic_delete_black_24dp)
                                mainFab.show()
                            }
                        }
                        2 -> {
                        ...
                        3 -> {
                            ...
                            it.mainFab.also { mainFab ->
                                mainFab.hide()
                                mainFab.setImageResource(R.drawable.ic_add_black_24dp)
                                mainFab.show()
                            }
                        }
                        else -> return
            ...
            it.mainFab.setOnClickListener { view ->
                when (tabNavHostFragment.navController.currentDestination?.id) {
                    R.id.tab0Fragment -> {
                        ...
                        Intent(view.context, EndlessService::class.java).also { intent ->
                            intent.action = Constants.IntentActions.OutgoingCall.name
