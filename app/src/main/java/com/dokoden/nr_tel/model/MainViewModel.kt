...
class MainViewModel(application: Application) : AndroidViewModel(application) {
    ...
    val callHold = TempRepository.callHold
    val isOnCalling = TempRepository.isOnCalling
    val isOnTransfer = TempRepository.isOnTransfer
    val callName = TempRepository.callName
    val callNumber = TempRepository.callNumber
    val callStatus = TempRepository.callState
    ...
    fun onClickDelete() {
        callNumber.value?.also {
            if (it.isNotEmpty()) {
                callNumber.value = it.substring(0, it.length - 1)
            }
        }
    }
    ...
    fun onCallTransfer(view: View) {
        Intent(view.context, EndlessService::class.java).also {
            it.action = Constants.IntentActions.TransferCall.name
            view.context.startService(it)
        }
    }

    fun onClickHold(view: View) {
        if (callHold.value == true) callHold.postValue(true) else callHold.postValue(true)
    }
